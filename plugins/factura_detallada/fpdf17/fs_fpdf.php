...
class PDF_MC_Table extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->fact_dev('FACTURA', '001-001-00000000'.$this->fdf_codigo);

        // Fecha factura y Codigo Cliente
        $this->addDate($this->fdf_fecha);
        $this->addClient('02/01/2025 al 02/01/2026');
    ...
    function addClient($ref)
    {
        ...
        $this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 3);
        $this->SetFont("Arial", "B", 10);
        $this->Cell(10, 3, utf8_decode(mb_strtoupper('VALIDEZ')), 0, 0, "C");
        $this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 9);
        $this->SetFont("Arial", "", 9);
    ...
    function addTotal()
    {
        if ($this->es_factura) {
            ...
            $this->Line($r1 + 14, $y1, $r1 + 14, $y2);
            $this->Line($r1 + 21, $y1 + 4, $r1 + 21, $y2);
            /*
            $this->Line($r1 + 51, $y1, $r1 + 51, $y2);
            $this->Line($r1 + 58, $y1 + 4, $r1 + 58, $y2);
            $this->Line($r1 + 76, $y1, $r1 + 76, $y2);
            $this->Line($r1 + 53, $y1 + 4, $r1 + 53, $y2);
            */
            $this->Line($r1 + 32, $y1, $r1 + 32, $y2);
            $this->SetXY($r1, $y1);
            $this->Cell(15, 4, utf8_decode(mb_strtoupper($this->idioma->neto)), 0, '', "C");
            $this->SetX($r1 + 22);
            $this->Cell(1, 4, utf8_decode(mb_strtoupper($this->idioma->iva)), 0, '', "C");
            /*
            $this->SetX($r1 + 51);
            ...
            */
            ...
            $this->Cell(24, 4, utf8_decode(mb_strtoupper($this->idioma->importes)), 0, '', "C");
             ...
             $this->Cell(74, 14, utf8_decode(mb_strtoupper('Contribuyente Negocio Popular')), 0, '', "C");
             
             $this->Cell(-74, 24, utf8_decode(mb_strtoupper('Régimen RIMPE')), 0, '', "C");
             
              $this->Cell(74, 45, utf8_decode(mb_strtoupper('IMPRENTA FREIRE RODRIGUEZ JANETH JACQUELINE | RUC 0911403962001 | AUT. 6274')), 0, '', "C");
        }
